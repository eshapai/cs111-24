...
public class RURacing {
    ...
    public static int trackLength(char[][] map)
    {
        int trackLength = 0;
        for (char[] row : map){
            for(char block:row){
    ...
    {
	    distance = trackLength(map);
        int currPosX = 0;
        int currPosY = 0;
        while (true)
        {
            int[] nextPos = moveRacer(currPosX, currPosY, distance, map);
            System.out.println("Racer1 is at: (" + nextPos[0] + "," + nextPos[1] + ")");
            currPosX = nextPos[0];
            currPosY = nextPos[1];
    ...
    {
        int distanceTotal = trackLength(map);
        int currPosX = 0;
        int currPosY = 0;
        distance = 1;
        ...
        while (true)
        {
            ...
            {
                int[] finalPos = moveRacer(currPosX, currPosY, distanceLeft, map);
                System.out.println("Racer2 is at: (" + finalPos[0] + "," + finalPos[1] + ")");
                break;   
            }
            int[] newPos = moveRacer(currPosX, currPosY, distance, map);
            System.out.println("Racer2 is at: (" + newPos[0] + "," + newPos[1] + ")");
            distanceLeft -= distance;
            currPosX = newPos[0];
            currPosY = newPos[1];
            distance = Math.min(distance*2, distanceLeft);
        }
    }
    ...
    public static void racer3(char[][] map) 
    {
        ...
        while (true)
        {
            int[] newPos = moveRacer(currPosX, currPosY, 1, map);
            System.out.println("Racer3 is at: (" + newPos[0] + "," + newPos[1] + ")");
            currPosX = newPos[0];
            currPosY = newPos[1];
            if (currPosX == 0 && currPosY == 0)
            {
    ...
    { 
        ...
        while (true)
        { 
            for (int i = 0; i < distanceTotal - 1; i++)
            {
                System.out.println("Racer4 is at: (" + currPosX + "," + currPosY + ")");
            }
            int[] newPos = moveRacer(currPosX, currPosY, 1, map);
            currPosX = newPos[0];
            currPosY = newPos[1];
            if (currPosX == 0 && currPosY == 0)
            {
                System.out.print("Racer4 is at: (0,0)");
                break;
            }
        }
    }
